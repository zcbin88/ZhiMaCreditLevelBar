...
import com.zcb.levelbarapplication.util.DensityUtil;
...
//        paint_bg.setStrokeCap(Paint.Cap.ROUND);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (cDialLenth > 0) {
            ...
            if (currentDialvalue >= 350 && currentDialvalue < 550) {
                ...
                draw5Characters(canvas, 1, text_pw2, text_ph);
            ...
            draw5Characters(canvas, 0, text_pw, text_ph);
     ...
     * @param text_ph
     */
    public void draw5Characters(Canvas canvas, int index, int text_pw, int text_ph) {
        ...
        for (int i = 1; i <= 5; i++) {
            if (i == index) {
                ...
                /////////若使用根据值进行定位则使用下面一段代码/////////////////////////////
                //圆角矩形
//                RectF oval3 = new RectF(text_pw-15,getPaddingTop()-20,text_pw+getTextWH("测试",textSize).width()+15,text_ph+15);// 设置个新的长方形
//                canvas.drawRoundRect(oval3,6,6,recPaint);
//
//                //三角形
//                Path path = new Path();
//                path.moveTo(text_pw, getPaddingTop());// 此点为多边形的起点
//                path.lineTo(text_pw+70, getPaddingTop());
//                path.lineTo(text_pw+30, text_ph+25);
//                path.close(); // 使这些点构成封闭的多边形
//                canvas.drawPath(path, recPaint);
//
//                paint_text_value.setColor(Color.WHITE);
//                canvas.drawText(paintString[i-1], text_pw, text_ph, paint_text_value);
////////////////////////////////////////////////////////////////////
